...
export function MenuItemCard({ item }: { item: MenuItem }) {
  ...
  const audioText = item.audio_text || `${item.name}. ${item.description || ""} Preço: ${item.price} reais. ${item.ingredients ? `Ingredientes: ${item.ingredients}` : ""}`;
  ...
  return (
    <>
      <article
        ...
        aria-label={`Prato: ${item.name}`}
      >
        {item.image_url && (
          <div className="aspect-[16/10] overflow-hidden">
            <img
              src={item.image_url}
              alt={item.image_alt || `Foto do prato ${item.name}`}
        ...
        <div className="p-4 space-y-3">
          ...
          {item.allergens && item.allergens.length > 0 && (
            <div className="flex flex-wrap gap-1.5" role="list" aria-label="Alérgenos">
          ...
          <div className="flex gap-2 pt-1">
            <button
              ...
              aria-label={`Ouvir descrição de ${item.name}`}
            >
              <Volume2 className="w-4 h-4" aria-hidden="true" />
              <span>Áudio</span>
            </button>

            {item.libras_video_url && (
              <button
                onClick={() => setLibrasOpen(true)}
                className="flex-1 flex items-center justify-center gap-2 py-2.5 px-3 rounded-md bg-secondary text-secondary-foreground font-medium text-sm hover:bg-secondary/80 transition-colors focus:outline-none focus:ring-2 focus:ring-ring"
                aria-label={`Ver vídeo em Libras de ${item.name}`}
              >
